...
async function parseClinicExport(env, key, documentId, text, pageCount, kinds) {
  ...
  let shaped = shapeParsed({
    ...
    date: (merged.invoices && merged.invoices[0]?.invoice_date) || latest?.event_date || latest?.date || harvested.date || segs[0]?.date || null,
    clinic: (merged.invoices && merged.invoices[0]?.clinic) || latest?.clinic || (datedVisits.length <= 1 ? (harvested.clinic || segs[0]?.clinic || null) : null),
    vet: (merged.invoices && merged.invoices[0]?.vet) || latest?.vet || null,
    issuing_clinic: (merged.invoices && merged.invoices[0]?.issuing_clinic) || exporting || null,
    document_date: (merged.invoices && merged.invoices[0]?.document_date) || document_date,
